...
export default function Dashboard() {
  ...
  // --- SEO Summary Variables for Featured Snippets ---
  const dominantPollutant = aqiData?.dominant_pollutant || "Unknown";
  const seoRecommendation = `${sensitiveGuidance.join(" ")} ${durationGuidance.join(" ")}`;
  const validForecasts = forecast.filter(f => f.aqi > 0);
  const minForecastAqi = validForecasts.length > 0 ? Math.min(...validForecasts.map(f => f.aqi)) : aqi;
  const maxForecastAqi = validForecasts.length > 0 ? Math.max(...validForecasts.map(f => f.aqi)) : aqi;
  const avgForecastAqi = validForecasts.length > 0 
    ? Math.round(validForecasts.reduce((sum, f) => sum + f.aqi, 0) / validForecasts.length) 
    : aqi;
  ...
  return (
    <motion.div
      ...
      animate="show"
    >
      {/* SEO-Optimized Invisible Text Block for Featured Snippets */}
      <section className="sr-only" aria-hidden="false">
        <h2>{city} AQI Summary</h2>
        <p>
          The current Air Quality Index in {city} is {aqi} which is categorised as {category}. 
          The dominant pollutant is {dominantPollutant}. {seoRecommendation}
        </p>
        <h2>{city} AQI Forecast Next 48 Hours</h2>
        <p>
          Over the next 48 hours, {city} AQI is forecast to range between {Math.round(minForecastAqi)} and {Math.round(maxForecastAqi)} 
          with an average of {avgForecastAqi}. Predictions are generated by an XGBoost ML model trained on India sensor data.
        </p>
        <h2>Health Recommendations for {city}</h2>
        <p>{seoRecommendation}</p>
      </section>
